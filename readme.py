...
def capture_from_camera_and_show_images():
    ...
    use_droid_cam = True
    if use_droid_cam:
        url = "http://10.209.89.165:4747/video"
    ...
    while not stop:
        ...
        binary_image = dif_img > THRESHHOLD

        F = np.sum(binary_image == 1)
        B = np.sum(binary_image == 0)

        percent_change = (F/(F+B))

        if (percent_change > ALERT):
            str_alert = "Change Detected"
        ...
        show_in_moved_window('Binary image', img_as_ubyte(binary_image), 600, 400)
